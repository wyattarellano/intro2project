...
;; A node is a structure, (make-node X (bstof X) (bstof X)), with an X value and two 
...
;; number bst --> string lod throws error
;; Purpose: To find the path of 'lefts and 'rights followed in a binary search tree to reach the given number
(define (find-path a-num a-bst)
  (if (= a-num (node-val a-bst))
      "The given number is the root of the binary tree."
      (local [;; number bst lod --> lod throws error
              ;; Purpose: To find the path of 'lefts and 'rights followed to reach the given number
              ;; Accumulator Invariant: path = List of the directions taken through the processed nodes of a
              ;;                        binary search tree to reach a given number
              (define (find-path-helper a-num a-bst path)
                (cond [(empty? a-bst) (error "number not found in tree")]
                      [(= a-num (node-val a-bst)) path]
                      [(< a-num (node-val a-bst))
                       (find-path-helper a-num (node-ltree a-bst) (append path '(left)))]
                      [(> a-num (node-val a-bst))
                       (find-path-helper a-num (node-rtree a-bst) (append path '(right)))]
                      [else '()]))]
        (find-path-helper a-num a-bst '()))))
...
(define SAMP-PATH2 (local [;; number bst lod --> lod throws error
                               ;; Purpose: To find the path of 'lefts and 'rights followed to reach the given number
                               ;; Accumulator Invariant: path = List of the directions taken through the processed nodes of a
                               ;;                        binary search tree to reach a given number
                               (define (find-path-helper a-num a-bst path)
...
(define SAMP-PATH3 "The given number is the root of the binary tree.")
...
(check-expect (find-path 79 (make-node 90
                                       (make-node 50 (make-node 45 (make-node 11 '() '()) (make-node 48 '() '()))
                                                  (make-node 60 (make-node 52 '() '()) (make-node 79 '() '())))
                                       (make-node 120 (make-node 115 (make-node 110 '() '()) (make-node 119 '() '()))
                                                  (make-node 165 (make-node 145 '() '()) (make-node 193 '() '())))))
...
(check-expect (find-path 15 (make-node 15 (make-node 10 (make-node 7 (make-node 3 '() '()) (make-node 9 '() '()))
                                       ...
                                       (make-node 73 (make-node 60 (make-node 50 '() '()) (make-node 63 '() '()))
                                                  (make-node 115 (make-node 95 '() '()) (make-node 162 '() '())))))
              "The given number is the root of the binary tree.")