...
(define (new-merge a-lon)
  (local [;; lon lon --> lon
    ...
    (if (empty? a-lon)
        '()
        (first (merge-sort-helper (create-lolon a-lon))))))

;;sample expressions
(define SAMP-NEW-MERGE-VAL1 (local [;; lon lon --> lon
                                    ;; Purpose: Merge the given lons in nondecreasing order
                                    ;; Assumption: Given lons are in nondecreasing order
                                    (define (merge l1 l2)
                                      (cond [(empty? l1) l2]
                                      ...
                                      (if (< (length a-lolon) 2)
                                          ...
                                          (local [(define NEW-LOLON (rest (rest a-lolon)))]
                                            (cons (merge (first a-lolon) (second a-lolon))
                                                  (merge-neighs NEW-LOLON)))))
                                    ;; lolon --> lolon
                                    ;; Purpose: Sort the numbers in the given lolon
                                    ;; How: If the length of the given lolon is 1 return it.
                                    ;;      Otherwise, every two neighboring lons are merged to
                                    ;;      create a new problem instance that is recursively
                                    ;;      processed.
                                    ;; Assumption: The given lolon has a length greater or equal to 1
                                    (define (merge-sort-helper a-lolon)
                                      (if (= (length a-lolon) 1)
                                          a-lolon
                                          (local [(define NEW-LOLON (merge-neighs a-lolon))]
                              ...
                              (if (empty? LON1)
                                  '()
                                  (first (merge-sort-helper (create-lolon LON1))))))

(define SAMP-NEW-MERGE-VAL2 (local [;; lon lon --> lon
                                    ;; Purpose: Merge the given lons in nondecreasing order
                                    ;; Assumption: Given lons are in nondecreasing order
                                    (define (merge l1 l2)
                                      (cond [(empty? l1) l2]
                              ...
                              (if (empty? LON2)
                                  '()
                                  (first (merge-sort-helper (create-lolon LON2))))))

(define SAMP-NEW-MERGE-VAL3 (local [;; lon lon --> lon
                                    ;; Purpose: Merge the given lons in nondecreasing order
                                    ;; Assumption: Given lons are in nondecreasing order
                                    (define (merge l1 l2)
                                      (cond [(empty? l1) l2]
                              ...
                              (if (empty? LON3)
                                  '()
                                  (first (merge-sort-helper (create-lolon LON3))))))

;;Tests using sample computations
(check-expect (new-merge LON1) SAMP-NEW-MERGE-VAL1)
(check-expect (new-merge LON2) SAMP-NEW-MERGE-VAL2)
(check-expect (new-merge LON3) SAMP-NEW-MERGE-VAL3)

;;Tests using sample values
(check-expect (new-merge (list 1 7 3 0 7 9)) (list 0 1 3 7 7 9))
(check-expect (new-merge (list 7 19 2 0 15 7)) (list 0 2 7 7 15 19))
(check-expect (new-merge (list 3 89 1 50 4 7)) (list 1 3 4 7 50 89))


;;Timing the functions
"New Merge"
(define T1-NM (time (new-merge LON1)))
(define T2-NM (time (new-merge LON2)))
(define T3-NM (time (new-merge LON3)))
(define T4-NM (time (new-merge LON4)))
(define T5-NM (time (new-merge LON5)))
(define T6-NM (time (new-merge LON6)))
(define T7-NM (time (new-merge LON7)))
"Old Merge"
...
#|
The refactored version of merge-sorting is not comparatively faster
than the old version. Both versions ran in 0 cpu time across all tests.
This showcases that they are comparatively similar.
It does not run much faster at any point because the cpu time is 0
across all time tests.
|#
